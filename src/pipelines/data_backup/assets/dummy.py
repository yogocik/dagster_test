...
@asset(
    ...
    check_specs=[AssetCheckSpec(name='non_empty_data', 
                                asset=AssetKey([*KEY_PREFIX, 'asset_1'])
                                ),
...
@asset(
    ...
    kinds={PYTHON_COMPUTE_KIND},
    ins={
        "upstream_asset": AssetIn(key=AssetKey([*KEY_PREFIX, 'asset_1']))
        },
    owners=[ANONYM, 'team:personal'],
    ...
    check_specs=[AssetCheckSpec(name='existed_data', 
                                asset=AssetKey([*KEY_PREFIX, 'asset_2'])
                                ),
                 ],
...
def processed_dummy_asset(context: AssetExecutionContext, 
    ...
    if isinstance(upstream_asset, dict):
        ...
        context.log.error(f"Processed data : {data}, Source : {upstream_asset}")
...
@asset(
    ...
    kinds={PYTHON_COMPUTE_KIND},
    ins={
        "upstream": AssetIn(key=AssetKey([*KEY_PREFIX, 'asset_2']))
        },
    owners=[ANONYM, 'team:personal'],
